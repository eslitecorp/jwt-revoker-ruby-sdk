...
module JwtRevoker
  ...
  class Client
    def initialize(config = JwtRevoker.config)
      ...
      @stub = Eslite::JwtRevoker::JwtRevoke::V1::JWTRevokerService::Stub.new(@config.endpoint, :this_channel_is_insecure)
    end
    ...
    def revoked?(token)
      ...
      response = @stub.check_revoked(request, deadline: Time.now.utc + @config.timeout)
